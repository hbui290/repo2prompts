...
projects use `scripts/bin/harness-cli` as the stable entrypoint. That path is a
small wrapper which finds or downloads the current-platform artifact into
`scripts/bin/harness-cli-<platform>` and then executes it.
...
full usage. On Windows, use `scripts/bin/harness-cli.cmd` or `npm run harness -- <args>`.
...
If the current-platform artifact is missing, run `npm run bootstrap` from the
repo root. The bootstrap flow retries dependency install, verifies native
modules, and downloads the matching Harness CLI artifact when possible.
...
current platform behind the stable `scripts/bin/harness-cli` entrypoint, then
...
that tag, `HARNESS_CLI_BASE_URL` to point at an alternate artifact directory,
or `HARNESS_CLI_BINARY_OVERRIDE` for local wrapper tests and emergency manual
overrides.
