...
`scripts/bin/harness-cli` is a stable repo-local wrapper. It resolves the
current-platform CLI artifact before executing commands, so users should keep
calling the same path even when the underlying binary differs by platform.
