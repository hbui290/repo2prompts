...
On Windows, the repository-local command is exposed as:

```powershell
.\scripts\bin\harness-cli.cmd <command>
```

The installer should download, verify, and install the platform-specific Rust
binary behind the stable repo-local entrypoint. The project may use a thin
wrapper at `scripts/bin/harness-cli` so the command contract stays stable while
the underlying artifact remains platform-specific.
...
- The `scripts/bin/harness-cli` command is the stable entrypoint for agents.
  A thin wrapper may resolve the current-platform artifact behind that command.
...
- Keep only a minimal stable entrypoint wrapper when needed; do not reintroduce
  business logic into shell glue.
